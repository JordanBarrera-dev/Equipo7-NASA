    ...
    fetch(url)
        ...
        .then(function(datos) {
            ...
            if (datos.media_type === "video") {
                contenido = `<iframe src="${datos.url}" style="width:100%; height:300px; border:none; border-radius:8px;"></iframe>`;
            } else {
        ...
        columna.innerHTML = `
            <div class="card h-100 bg-dark bg-opacity-50 border-secondary">
                ...
                <div class="card-body">
                    ...
                    <button class="btn btn-sm btn-outline-primary w-100" onclick="cargarFecha('${fav.date}')">
                        Ver imagen
                    </button>
                </div>
            </div>
...
function cargarFecha(fechaElegida) {
    const inputFecha = document.getElementById("fecha-nasa");
